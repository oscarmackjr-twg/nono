...
mod windows_impl {
    ...
    use windows_sys::Win32::Foundation::{
        CloseHandle, GetLastError, LocalFree, ERROR_ACCESS_DENIED, GENERIC_READ, GENERIC_WRITE,
    ...
    fn run_parent(args: &[String]) -> ExitCode {
        ...
        // Diagnostic: dump the restricted token's groups + restricting SIDs so we
        // can see what Windows actually stored vs. what we asked for.
        if env::var_os("PIPE_REPRO_DUMP_TOKEN").is_some() {
            dump_token_sids("restricted_token", restricted_token);
        }
        ...
        if child_exit_code == 0 {
            println!(
                "RESULT: CreateFileW succeeded — this SDDL shape ADMITS a WRITE_RESTRICTED child."
            );
            ExitCode::SUCCESS
    ...
    fn create_restricted_token_with_sid(session_sid: &str) -> Result<HANDLE, String> {
        ...
        if ok == 0 {
            return Err(format!("CreateRestrictedToken failed: err={}", unsafe {
                GetLastError()
            }));
        }
    ...
    /// Diagnostic helper. Queries and prints both `TokenGroups` and
    /// `TokenRestrictedSids` from a token so we can see what Windows actually
    /// stored vs. what we asked for when building the restricted token.
    ///
    /// Gated by the `PIPE_REPRO_DUMP_TOKEN` env var; opt-in because the output is
    /// noisy.
    fn dump_token_sids(label: &str, h_token: HANDLE) {
        use windows_sys::Win32::Security::Authorization::ConvertSidToStringSidW;
        use windows_sys::Win32::Security::{
            GetTokenInformation, TokenGroups, TokenRestrictedSids, SID_AND_ATTRIBUTES as SA,
            TOKEN_GROUPS,
        };

        for (class, class_label) in [
            ...
            let _ = unsafe {
                GetTokenInformation(h_token, class, std::ptr::null_mut(), 0, &mut needed)
            };
            if needed == 0 {
                println!("{label}: {class_label}: <empty>");
                continue;
            }
            let mut buf = vec![0u8; needed as usize];
            let ok = unsafe {
                // SAFETY: buf is sized from the probe above; h_token is live.
                GetTokenInformation(
                    h_token,
                    class,
            ...
            if ok == 0 {
                println!("{label}: {class_label}: <GetTokenInformation failed>");
                continue;
            }
            // SAFETY: TOKEN_GROUPS header is followed by a variable-length
            // SID_AND_ATTRIBUTES[] (GroupCount elements).
            let groups = unsafe { &*(buf.as_ptr() as *const TOKEN_GROUPS) };
            let count = groups.GroupCount as usize;
            println!("{label}: {class_label}: count={count}");
            // The Groups field in TOKEN_GROUPS is declared as [SID_AND_ATTRIBUTES; 1]
            // but is variable-length. Walk it explicitly.
            let base = std::ptr::addr_of!(groups.Groups) as *const SA;
            for i in 0..count {
                // SAFETY: i is bounded by GroupCount; the array is sized correctly
                // by GetTokenInformation above.
                let entry = unsafe { &*base.add(i) };
                let mut sid_str: *mut u16 = std::ptr::null_mut();
                // SAFETY: entry.Sid is a valid SID pointer from the token's buffer.
                let ok = unsafe { ConvertSidToStringSidW(entry.Sid, &mut sid_str) };
                let sid_text = if ok != 0 && !sid_str.is_null() {
                    let mut len = 0;
                    while unsafe { *sid_str.add(len) } != 0 {
                        len += 1;
                    }
                    let slice = unsafe { std::slice::from_raw_parts(sid_str, len) };
                ...
                println!("  [{i:2}] attrs=0x{:08x} sid={sid_text}", entry.Attributes);
            }
        }
    }
} // mod windows_impl
