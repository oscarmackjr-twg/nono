...
On Windows, the current product contract is intentionally narrower than Linux or macOS: direct restricted execution, blocked-network and selected supervised flows are supported where the backend can enforce them credibly, while live `shell` and `wrap` remain intentionally unavailable. See [Security Model](/cli/internals/security-model) and [Windows Security Model](/cli/development/windows-security-model) for the exact boundary.
