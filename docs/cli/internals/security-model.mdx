...
For the current Windows filesystem boundary and parity contract, see [Windows Filesystem Parity Contract](/cli/development/windows-filesystem-parity-contract). Windows currently exposes a narrower restricted-execution command surface than the Linux and macOS kernel-enforced models described below.

Windows network enforcement now follows a separate backend-owned WFP contract rather than the Linux/macOS kernel path described on this page. For supported Windows runs, `nono-wfp-service` installs deterministic per-app WFP permit/block filters for blocked, proxy-only, connect-port, bind-port, and localhost-only policies, then removes that same filter set on cleanup. The current contract is still narrower than Linux/macOS because it depends on explicit backend readiness and service-managed activation instead of a single unprivileged kernel primitive.
...
The proxy runs in the unsandboxed parent process alongside the supervisor. On Linux and macOS, the sandboxed child can only reach `localhost:<port>` because the kernel policy blocks the rest. On Windows, the same product contract is enforced by the WFP backend: the service installs loopback-only permit filters for the proxy port and fallback block filters for the remaining traffic surface. A session token (256-bit random) prevents other localhost processes from using the proxy.
...
Windows currently uses a narrower restricted-execution command surface than Linux or macOS.

The current Windows filesystem/runtime path combines:

- backend-owned executable and argument-path validation
- exact-path file grants plus directory allowlists in the compiled Windows policy
- low-integrity child launches for a real write boundary against ordinary medium-integrity host paths
- writable runtime redirection into low-integrity-compatible roots when direct writes are not credible

Two limitations matter:

- Windows does not yet provide a general backend-native deny-within-allow filesystem rule comparable to Seatbelt
- launch-time path mediation is still preflight, not a complete kernel-level substitute for broader filesystem enforcement

That means the Linux and macOS models remain the reference design for full kernel-enforced sandbox behavior, while Windows is an intentionally explicit restricted command surface with fail-closed diagnostics where parity is not yet credible.
...
That split is intentional until the library/runtime boundary is aligned. It prevents release and support messaging from overstating Windows as a full peer to Linux or macOS before the lower-level support contract says the same thing.
