...
For the current Windows filesystem boundary and parity contract, see [Windows Filesystem Parity Contract](/cli/development/windows-filesystem-parity-contract). Windows uses a narrower native subset than the Linux and macOS kernel-enforced models described below.
...
Windows currently uses a narrower native restricted-run subset than Linux or macOS.
...
That means the Linux and macOS models remain the reference design for full kernel-enforced sandbox behavior, while Windows is an intentionally explicit native subset with fail-closed diagnostics where parity is not yet credible.
