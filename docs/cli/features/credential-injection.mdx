...
## Custom Keyring Entries (`keyring://` URIs)

For credentials stored in the system keyring under a custom service name (rather than the default `nono` service used by `--env-credential`), use a `keyring://` URI. This is useful when you want to share a keyring entry with another tool that owns the service name, or when integrating with existing secret-management workflows.

URI format:

```text
keyring://<service>/<account>
keyring://<service>/<account>?decode=go-keyring
```

- `service`: the keyring service name (for example, `my-tool`, `docker-credential-helpers`)
- `account`: the account/username under that service
- `?decode=go-keyring` (optional): decode values stored by Go's `go-keyring` package, which wraps raw secrets in a base64 JSON envelope. Use this when integrating with Go tools that persisted their secrets through `go-keyring`.

The parser is fail-closed: malformed URIs, path-traversal inputs (`keyring://../../../etc/shadow`), unknown codecs (`?decode=unknown`), and oversize URIs (> 1024 bytes) are rejected at config-load time without touching the filesystem.

### CLI

```bash
# Load from a custom-service keyring entry
nono run --allow . --env-credential-map 'keyring://my-tool/api-key' MY_TOOL_API_KEY -- my-agent

# Decode a go-keyring-wrapped value
nono run --allow . --env-credential-map 'keyring://docker-credentials/ghcr.io?decode=go-keyring' GHCR_TOKEN -- my-agent
```
...
```json
{
  "meta": { "name": "my-agent" },
  "env_credentials": {
    "keyring://my-tool/api-key": "MY_TOOL_API_KEY"
  }
}
```

<Note>
  `keyring://` differs from plain `--env-credential <NAME>` in that it lets you specify the keyring **service name** explicitly. Plain `--env-credential openai_api_key` always resolves under the `nono` service; `keyring://my-tool/openai_api_key` resolves under the `my-tool` service.
</Note>
