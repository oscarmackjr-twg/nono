...
```
((default_profile_groups + profile.security.groups) - profile.policy.exclude_groups) + profile.filesystem + CLI flags
```

1. **default_profile_groups** come from the built-in `default` profile
2. **profile.policy.exclude_groups** removes groups from the composed set
...
**Groups:** `claude_code_macos`, `claude_code_linux`, `user_caches_macos`, `node_runtime`, `rust_runtime`, `python_runtime`, `vscode_macos`, `vscode_linux`, `unlink_protection` (plus the built-in `default` profile groups)
...
**Groups:** `node_runtime`, `rust_runtime`, `python_runtime`, `nix_runtime`, `unlink_protection` (plus the built-in `default` profile groups)
