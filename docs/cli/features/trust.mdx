...
Creates an ECDSA P-256 key pair in the system keystore (Windows Credential Manager / macOS Keychain / Linux Secret Service).
...
On Windows, this startup verification is the currently supported trust boundary. Runtime trust interception inside supervised child execution is not yet available because Windows supervised child processes do not yet have an attached file-open mediation channel.
...
nono also supports GitLab ID tokens for keyless signing on GitLab-hosted pipelines. The flow mirrors GitHub Actions: the runner exposes an ambient OIDC token, `nono trust sign --keyless` exchanges it for a Fulcio certificate, and the resulting bundle is verified against a `gitlab.com` issuer pin in your trust policy.
...
### Trust Policy for GitLab-Signed Files
...
For **self-hosted GitLab**, set `issuer` to your instance URL (e.g., `"https://gitlab.example.com"`). The issuer validator compares URL components (scheme, host, port, path) using `url::Url::parse`, not string prefix-match, so `https://gitlab.example.com.evil.com` is rejected even if it starts with your expected hostname.

<Note>
  The issuer URL is matched fail-closed: a component-equality mismatch (scheme, host, port, or path) rejects the certificate. This prevents prefix-match and subdomain-hijack attacks where an attacker-controlled issuer shares a prefix with a trusted one.
</Note>
...
**Keyed signing** — A private key stored in the system keystore (Windows Credential Manager / macOS Keychain / Linux Secret Service). Suitable for individual developers and local workflows.
