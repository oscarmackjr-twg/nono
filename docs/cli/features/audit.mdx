...
Audit sessions are stored in the platform rollback directory (`~/.nono/rollbacks/` on Unix-like systems, the nono state directory on Windows). Audit-only sessions are tiny (a single `session.json` file) and are not subject to rollback storage limits. They accumulate freely and can be cleaned up with `nono rollback cleanup`.
