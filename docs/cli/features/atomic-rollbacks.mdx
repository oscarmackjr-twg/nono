...
<Note>
`--rollback` automatically selects supervised execution because the parent process needs to remain unsandboxed to write snapshots to the platform rollback directory (`~/.nono/rollbacks/` on Unix-like systems, the nono state directory on Windows).
</Note>

On Windows, rollback state lives under the nono state directory rather than a Unix-style home-directory fallback. `nono setup --check-only` reports the exact config root, state root, and rollback root that the current installation will use.
...
Sessions are stored in the platform rollback directory with the following layout:
...
Session IDs use the format `YYYYMMDD-HHMMSS-PID` (e.g., `20260214-143022-12345`). Older Windows sessions may also appear as `windows-preview-<pid>-<timestamp>`, and nono treats both formats as valid when detecting stale sessions.
...
Restoration uses an atomic replace step per file to prevent partial restores. On Windows, this uses Windows-safe replace semantics rather than assuming Unix rename behavior.

If a file cannot be restored because it is locked or in use by another process on Windows, restore now fails explicitly with a Windows-aware diagnostic instead of silently downgrading that file to a warning.
