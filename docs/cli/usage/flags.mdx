...
On Windows, live `nono shell` execution is not supported. Use `nono shell --dry-run` to inspect the resolved policy or `nono run -- <command>` for supported execution.
...
On Windows, live `nono wrap` execution is not supported. Use `nono wrap --dry-run` to inspect the resolved wrap policy or `nono run -- <command>` for supported execution.
