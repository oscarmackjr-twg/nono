...
| `nono shell` | Start an interactive shell inside the sandbox (`--dry-run` only on Windows) |
| `nono wrap` | Apply sandbox and exec into command (no parent process, minimal overhead; `--dry-run` only on Windows) |
...
| `nono learn` | [Discover required paths](/cli/features/learn) - trace a command to find what it accesses |
...
<Note>
On Windows, live `nono shell` execution is intentionally unavailable as a product-surface limitation. Use `nono shell --dry-run` to inspect shell policy, or use `nono run -- <command>` for supported execution.
</Note>
