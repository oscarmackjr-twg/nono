...
| `nono wrap` | Apply sandbox and exec into command (no parent process, minimal overhead; not supported live on Windows) |
...
<Note>
On Windows, live `nono shell` execution is not supported. Use `nono shell --dry-run` to inspect shell policy, or use `nono run -- <command>` for supported execution.
</Note>
