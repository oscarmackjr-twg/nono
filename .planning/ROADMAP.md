...
| 21. Windows Single-File Filesystem Grants | v2.1 | 2/5 | In progress (21-01 bookkeeping complete 2026-04-20 commits `d8dbe2c`+`6df4f4b`; 21-02 enforcement primitive complete 2026-04-20 commits `1a545e1`+`d19aaaa`+`853683a`+`7637694` — NonoError::LabelApplyFailed variant + three pub label helpers `try_set_mandatory_label`/`label_mask_for_access_mode`/`low_integrity_label_and_mask` + `pub mod windows` promotion + lib.rs re-exports under `#[cfg(target_os = "windows")]` + nono-ffi map_error arm auto-fix; TDD RED/GREEN gates observed for both tasks; windows-sys 0.59 module paths locked; D-21 Windows-invariance held; cargo build/test/clippy/fmt green workspace-wide. Implementation 21-03..21-05 pending) | — |
