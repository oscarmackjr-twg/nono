...
- [ ] **Phase 21: Windows Single-File Filesystem Grants** — Extend the Windows filesystem sandbox backend in `crates/nono/src/sandbox/windows.rs` to enforce single-file capability grants (read / write / read-write). Today `WindowsUnsupportedIssueKind::SingleFileGrant` makes any `CapabilitySet` containing file-scoped grants (e.g. the `git_config` policy group: `.gitconfig`, `.gitignore_global`, `.config/git/config`, `.config/git/ignore`, `.config/git/attributes`) fail to launch on Windows under the `claude-code` profile. Design the enforcement primitive (per-file DACL vs alternative), preserve fail-closed semantics on any error, and land platform-split tests proving the grants do not silently degrade to broader directory grants.
  ...
  **Open questions** (for `/gsd-plan-phase 21` or `/gsd-discuss-phase 21`):
  - Enforcement primitive: per-file DACL vs existing directory-scope mechanism with a file filter — trade-off between precision and SDDL surface area.
  - Interaction with existing deny rules and `WindowsUnsupportedIssueKind::WriteOnlyDirectoryGrant` — should write-only single-file grants also be supported, or scoped out for a follow-up?
  - Test strategy: how to exercise the enforcement from userland (`nono run --profile claude-code --allow-file <path>`) without admin privileges.

  **Plans:** TBD during `/gsd-plan-phase 21`.
...
| 21. Windows Single-File Filesystem Grants | v2.1 | 4/5 | In progress (21-01 bookkeeping complete 2026-04-20; 21-02 enforcement primitive complete 2026-04-20; 21-03 apply() wiring complete 2026-04-20 commits `2054903`+`a59e978`+`5526068`+`8c47a6b` — `compile_filesystem_policy` emits rules for single-file Read/Write/ReadWrite + write-only-dir (no `unsupported.push`), `apply()` iterates rules + calls `try_set_mandatory_label` fail-closed, 4 pre-existing rejection tests flipped to acceptance tests + IPC-mode unsupported test repointed, `WINDOWS_SUPPORTED_DETAILS` surface updated, D-06 enum-shape preservation held; 21-04 AppliedLabelsGuard RAII complete 2026-04-20 commits `3ad4f64`+`bedf679` — 378-line labels_guard.rs with Applied/Skip entries + revert-on-Drop + skip-on-any-prior-label (tracing::warn!) + snapshot-before-apply invariant + 3 lifecycle unit tests + wiring into `prepare_live_windows_launch`/`PreparedWindowsLaunch` alongside `_network_enforcement`; TDD RED/GREEN gates observed; D-21 Windows-invariance held; cargo test/clippy/fmt green. 21-05 tests+HUMAN-UAT pending) | — |
