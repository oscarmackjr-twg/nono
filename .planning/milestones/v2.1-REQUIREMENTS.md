...
**Status:** ✅ SHIPPED 2026-04-21. All 13 requirements closed. Snapshot preserved below as the authoritative v2.1 acceptance record. For v2.2+ work, see the live `.planning/REQUIREMENTS.md` in the milestone root.

## Traceability (final)

| ID | Requirement | Phase | Status |
|----|-------------|-------|--------|
| RESL-01 | CPU percentage cap via `JOB_OBJECT_CPU_RATE_CONTROL_HARD_CAP` | 16 | ✅ Complete |
| RESL-02 | Memory cap via `JobMemoryLimit` + `KILL_ON_JOB_CLOSE` | 16 | ✅ Complete |
| RESL-03 | Wall-clock timeout via supervisor-side `Instant` + `TerminateJobObject` | 16 | ✅ Complete |
| RESL-04 | Process count cap via `ActiveProcessLimit` | 16 | ✅ Complete |
| AIPC-01 | Extended handle brokering (Socket/Pipe/JobObject/Event/Mutex) | 18 + 18.1 | ✅ Complete (HUMAN-UAT item 3 WR-02 EDR deferred to v3.0) |
| ATCH-01 | `nono attach <id>` on detached Windows sessions | 17 | ✅ Complete (acceptance #3 resize downgraded per D-07) |
| CLEAN-01 | `cargo fmt --all` drift fix + CI gate | 19 | ✅ Complete |
| CLEAN-02 | 5 Windows test flakes root-caused + fixed | 19 | ✅ Complete-with-deviation (option C scope expansion — `query_path` UNC prod fix) |
| CLEAN-03 | Disk-resident WIP triage | 19 | ✅ Complete (10 items: 6 backfilled, 2 reverted, 2 deleted) |
| CLEAN-04 | Session-file retention policy + prune | 19 | ✅ Complete (1343-file one-shot cleanup + auto-sweep + docs) |
| UPST-01 | rustls-webpki 0.103.12 security upgrade + workspace 0.37.1 | 20 | ✅ Complete |
| UPST-02 | Profile `extends` cycle guard + `.claude.json` symlink | 20 | ✅ Complete |
| UPST-03 | `keyring://` URIs + env-var filtering + command_blocking_deprecation | 20 | ✅ Complete |
| UPST-04 | `--allow-gpu` + NVIDIA Linux allowlist + GitLab ID tokens | 20 | ✅ Complete |
| WSFG-01 | Per-file + write-only-dir Low-IL mandatory-label enforcement | 21 | ✅ Complete |
| WSFG-02 | `NonoError::LabelApplyFailed` + `AppliedLabelsGuard` RAII revert | 21 | ✅ Complete |
| WSFG-03 | Phase 18 HUMAN-UAT close-out (Path B + Path C) | 21 | ✅ Closed-with-deviation (frontmatter transition achieved; live verdicts folded into Phase 18.1) |

---

**Original requirements snapshot below (frozen at milestone close).**
